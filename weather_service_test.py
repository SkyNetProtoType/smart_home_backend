...
class WeatherServiceTest(unittest.TestCase):
    ...
    def test_getHourlyInfo_for_only_current_hour_returns_current_hour_data(self, mock_reqs):
        """
            test to return the current weather for the hour
        """
        localData = WeatherServiceTest.setUpClass()
        currentWeatherData = localData["current"]
        hourlyWeatherData = localData["hourly"]
        dailyWeatherData = localData["daily"]

        # self.as

        print(WeatherService.get_current_report())

    def test_getHourlyInfo_for_next_3_hours_returns_3_hours_data(self):
        """
            test to return the current weather plus the next 3 hours
